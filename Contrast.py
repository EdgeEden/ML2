...
class Config:
    ...
    synonym_file = "syno_light.txt"  # 同义词文件路径
    antonym_file = "anto_light.txt"
...
class AdversarialTrainer:
    ...
    def train_step(self, word2idx, pos_batch, neg_batch, optimizer):
        # 转换输入时添加异常处理
        try:
            anchor_pos = torch.tensor([word2idx[p[0]] for p in pos_batch])
            positive = torch.tensor([word2idx[p[1]] for p in pos_batch])
            anchor_neg = torch.tensor([word2idx[n[0]] for n in neg_batch])
            negative = torch.tensor([word2idx[n[1]] for n in neg_batch])
        ...
        # 计算损失时添加数值保护
        with torch.autocast(device_type='cuda'):  # 如果使用GPU则启用混合精度
...
def evaluate(model, dataset):
    ...
    with torch.no_grad():
        for pos, neg in dataset.get_batches():
            # 正样本计算
            anchor_pos = torch.tensor([dataset.word2idx[p[0]] for p in pos])
            positive = torch.tensor([dataset.word2idx[p[1]] for p in pos])
            pos_emb = model(anchor_pos)
            pos_emb_p = model(positive)
            pos_sims.extend(torch.cosine_similarity(pos_emb, pos_emb_p).tolist())

            # 负样本计算
            anchor_neg = torch.tensor([dataset.word2idx[n[0]] for n in neg])
            negative = torch.tensor([dataset.word2idx[n[1]] for n in neg])
            neg_emb = model(anchor_neg)
            neg_emb_n = model(negative)
            neg_sims.extend(torch.cosine_similarity(neg_emb, neg_emb_n).tolist())
...
if __name__ == "__main__":
    ...
    model = EnhancedWord2Vec(dataset.word2idx, dataset.wv)
    ...
    saved_vectors = model.embedding.weight.data.numpy()
